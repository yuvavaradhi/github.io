...
-- Automated User Creation Trigger for Supabase Auth Signup (Default Role: 'citizen')
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $
BEGIN
    INSERT INTO public.profiles (id, email, full_name, username, mobile_hashed, dob, role, is_active)
    VALUES (
        NEW.id,
        NEW.email,
        COALESCE(NEW.raw_user_meta_data->>'full_name', 'Citizen User'),
        COALESCE(NEW.raw_user_meta_data->>'username', SPLIT_PART(NEW.email, '@', 1)),
        COALESCE(NEW.raw_user_meta_data->>'mobile_hashed', '0000000000'),
        CURRENT_DATE,
        'citizen',
        TRUE
    )
...
-- RLS Policies for Profiles: Root Super Admin yuvavaradhi1@gmail.com has Full Administrative Authority
...
CREATE POLICY "users_read_own_profile"
    ...
    FOR SELECT
    TO authenticated
    USING (auth.uid() = id OR auth.email() = 'yuvavaradhi1@gmail.com' OR public.is_super_admin());
...
CREATE POLICY "users_update_own_profile"
    ...
    TO authenticated
    USING (auth.uid() = id)
    WITH CHECK (auth.uid() = id AND role IN ('student', 'faculty', 'citizen'));
...
CREATE TABLE IF NOT EXISTS public.student_innovation_pitches (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    student_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
    student_alias VARCHAR(50) NOT NULL, -- e.g. Innovator #NAT-418 (Zero PII)
...
('prog_school', 'School Education (Class 5th to 10th)', 'school', 'State Boards & Central CBSE/ICSE Foundation Track', 1),
('prog_inter', 'Intermediate (10+2 Higher Secondary)', 'intermediate', 'Board of Intermediate Education (BIE) Academic Streams', 2),
('prog_poly', 'Polytechnic / Diploma (3-Year Technical)', 'polytechnic', 'State & Central Technical Education Boards (AICTE/DTE) C-20/C-23', 3),
...
-- School
('br_sch_state', 'prog_school', 'SSC_STATE', 'State Boards (SSC)', 'state_ssc', 1),
...
(
    ...
    1200.00,
    250.00,
    'Central Agro Service Center (ASC-03)',
    4,
    'National Agro Machinery Corporation'
),
(
...
(
    'TOKEN-AGRI-4821',
    'Agri-Scholar #NAT-12',
    'ICAR National Agricultural College',
...
-- Seed Daily Mandi Rates for National APMC Yards
...
('Red Chilli (మిరప)', 'Teja Deluxe', 'Central Zone', 'National Spices Terminal & Wholesale APMC', 48500.00, 16800.00, 21400.00, 19800.00, NULL, 'up', 450.00, CURRENT_DATE),
('Cotton (ప్రత్తి)', 'Medium Staple H-4', 'Central Zone', 'Central Cotton Exchange & APMC Hub', 22400.00, 6800.00, 7450.00, 7250.00, 7120.00, 'up', 130.00, CURRENT_DATE),
('Paddy (వరి)', 'BPT 5204 (Samba Masoori)', 'Northern Zone', 'National Grain Terminal & APMC Yard', 34200.00, 2280.00, 2460.00, 2380.00, 2320.00, 'stable', 0.00, CURRENT_DATE),
...
('Bengal Gram (శనగలు)', 'Desi Bold', 'Western Zone', 'National Pulses & Oilseeds APMC', 11200.00, 5500.00, 6250.00, 5920.00, 5440.00, 'up', 80.00, CURRENT_DATE),
...
('BUYER-CORP-101', 'National Modern Rice Industries Ltd', 'Rice Mill', 'LIC-NAT-RM-4921', '37AABCS1234A1Z5', TRUE, '[{"commodity":"Paddy","variety":"BPT 5204","targetPrice":2420,"minQty":50,"maxMoisture":14}]'::jsonb, '["Northern Zone","Central Zone","Western Zone"]'::jsonb, 'P. Mallikarjuna Rao', 'procure@srilalitharice.com', 2500000.00),
('BUYER-CORP-102', 'ITC Agri-Business Division (Spices & Agri)', 'Export House', 'LIC-IND-EXP-8812', '36AAACI0002L1ZQ', TRUE, '[{"commodity":"Chilli","variety":"Teja Deluxe","targetPrice":20000,"minQty":20,"maxMoisture":11}]'::jsonb, '["Central Zone","Eastern Zone","Southern Zone"]'::jsonb, 'K. Srinivasa Murthy', 'spices.procure@itc.in', 5000000.00),
('BUYER-CORP-103', 'Central Mega Cotton Ginning & Pressing Mills', 'Cotton Ginning', 'LIC-NAT-COT-5509', '36AABCW9876C1ZB', TRUE, '[{"commodity":"Cotton","variety":"Medium Staple","targetPrice":7350,"minQty":30,"maxMoisture":8}]'::jsonb, '["Central Zone","Eastern Zone","Western Zone"]'::jsonb, 'B. Ramesh Chandra', 'cotton@centralginning.co.in', 3500000.00),
('BUYER-CORP-104', 'Apex Foods & Agronomy Dairy Feed Ltd', 'FMCG Processor', 'LIC-NAT-FMCG-7714', '37AAACH2244K1ZS', TRUE, '[{"commodity":"Maize","variety":"Yellow Hybrid","targetPrice":2220,"minQty":40,"maxMoisture":12}]'::jsonb, '["Eastern Zone","Northern Zone","Central Zone"]'::jsonb, 'Dr. V. Sudhakar', 'agriprocure@heritagefoods.in', 1800000.00),
('BUYER-CORP-105', 'Central Spices & Organic Turmeric Export Ltd', 'Export House', 'LIC-NAT-TUR-3381', '36AABCN4411D1ZP', TRUE, '[{"commodity":"Turmeric","variety":"Salem Finger","targetPrice":13900,"minQty":25,"maxMoisture":10}]'::jsonb, '["Northern Zone","Central Zone","Western Zone"]'::jsonb, 'M. Janardhan Reddy', 'export@nizamturmeric.com', 3000000.00),
('BUYER-CORP-106', 'National Pulses & Processing Corporation', 'Agri-Processing Hub', 'LIC-NAT-PLS-2201', '37AABCR6633E1ZM', TRUE, '[{"commodity":"Bengal Gram","variety":"Desi Bold","targetPrice":6050,"minQty":20,"maxMoisture":9.5}]'::jsonb, '["Western Zone","Southern Zone","Central Zone"]'::jsonb, 'S. Vijay Bhaskar', 'orders@rayalaseemapulses.com', 2200000.00)
ON CONFLICT DO NOTHING;
...
('CROP-7821', 'M. Venkateswarlu', '+91 98480 •••••', 'Central Zone', 'Sector-4', 'Hub-12', 'Chilli', 'Teja Deluxe', 85.00, 10.50, 19900.00, CURRENT_DATE - 2, 'Opp. National Agri Service Center, Sector-4', 'active'),
...
('CROP-8832', 'T. Koteswara Rao', '+91 99892 •••••', 'Central Zone', 'Sector-2', 'Hub-08', 'Cotton', 'Medium Staple', 140.00, 7.80, 7300.00, CURRENT_DATE - 3, 'Adjacent to Central Farmers Cooperative Society', 'active'),
('CROP-6014', 'G. Anji Reddy', '+91 96183 •••••', 'Western Zone', 'Sector-5', 'Hub-19', 'Bengal Gram', 'Desi Bold', 95.00, 9.00, 6000.00, CURRENT_DATE - 4, 'Banavasi Farm Road, Beside Borewell #4', 'active')
ON CONFLICT DO NOTHING;
...
CREATE TABLE IF NOT EXISTS public.sports_disciplines (
    ...
    federation_accredited BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.sports_academies (
    ...
    zone VARCHAR(50) NOT NULL CHECK (zone IN ('Northern Zone', 'Southern Zone', 'Western Zone', 'Eastern Zone', 'Central Zone', 'North-Eastern Zone')),
    district VARCHAR(100) NOT NULL,
...
('acad_sai_hyd', 'SAI National Sports Excellence Center', 'disc_athletics', 'Central Zone', 'Central Sports Hub', 'K. Prabhakar Rao (NIS Coach)', 'SAI-A-LIC-8812', '400m 8-lane synthetic track, Olympic gym, sports medicine center', '+91 98480 •••••', 'SAI-NAT-HYD-01'),
('acad_aca_vizag', 'National Cricket Excellence Academy', 'disc_cricket', 'Eastern Zone', 'Eastern Coast Hub', 'M. Venkatesh (BCCI Level 3)', 'BCCI-L3-4912', '12 turf practice wickets, indoor video analytics facility, floodlights', '+91 94401 •••••', 'NAT-CRIC-02'),
('acad_gopichand', 'National Badminton High-Performance Academy', 'disc_badminton', 'Southern Zone', 'Apex Training Complex', 'P. Gopichand (Dronacharya)', 'BWF-MASTER-001', '8 air-conditioned wooden-synthetic courts, biomechanics testing lab', '+91 99892 •••••', 'NAT-BADM-03'),
('acad_nat_athletics', 'National Athletics Excellence Center', 'disc_athletics', 'Central Zone', 'Apex Track Arena', 'G. Subba Rao (NIS Gold Medalist)', 'NAT-NIS-5521', 'B.R. Stadium 8-lane track, steeplechase water pit, high jump mats', '+91 96183 •••••', 'NAT-ATH-04'),
('acad_ts_sports_school', 'National Sports Academy & School', 'disc_volleyball', 'Central Zone', 'National Sports Center', 'V. Ravinder Reddy (FIVB Level 2)', 'FIVB-L2-3319', 'Indoor wooden volleyball stadium, clay outdoor courts, strength pool', '+91 97014 •••••', 'NAT-VB-05'),
('acad_nat_kabaddi', 'National Kabaddi & Wrestling Center', 'disc_kabaddi', 'Western Zone', 'National Arena', 'S. Mallikarjuna (PKL Certified)', 'AKFI-PRO-2281', 'International standard foam mat arena, clay akhada, recovery sauna', '+91 95532 •••••', 'NAT-KBD-06'),
('acad_nat_khokho', 'National Kho-Kho Training Academy', 'disc_khokho', 'Central Zone', 'Central Sports Grid', 'T. Srinivas (National Coach)', 'KKFI-NAT-1190', 'J.N. Stadium grass field, pole dive sand pits, agility training grid', '+91 93910 •••••', 'NAT-KHK-07'),
('acad_nat_volleyball', 'National Volleyball Academy', 'disc_volleyball', 'Northern Zone', 'Northern Sports Complex', 'D. Nageswara Rao (NIS)', 'VFI-NIS-4402', 'Indira Gandhi Stadium wooden court, beach volleyball training sand pit', '+91 98495 •••••', 'NAT-VB-08')
ON CONFLICT (id) DO NOTHING;


-- ============================================================================
-- 15. CITIZEN GRIEVANCE REDRESSAL DESK & SECURE VAULT
-- Dual-Delivery Architecture: Dispatched to yuvavaradhi1@gmail.com + Vault Store
-- ============================================================================

CREATE TABLE IF NOT EXISTS public.grievances (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tracking_id VARCHAR(30) UNIQUE NOT NULL,               -- e.g. YV-GRV-849201
    category VARCHAR(100) NOT NULL,                       -- Selected category/department
    description TEXT NOT NULL,                            -- Detailed complaint description
    citizen_name VARCHAR(150) NOT NULL,                   -- Unmasked confidential petitioner name
    citizen_phone VARCHAR(20) NOT NULL,                   -- Contact phone (for official admin contact only)
    citizen_name_masked VARCHAR(60),                      -- Masked petitioner name for public lookup
    citizen_phone_masked VARCHAR(20),                     -- Masked contact phone for public lookup
    district VARCHAR(100) DEFAULT 'National Jurisdiction',-- Jurisdiction / District
    status VARCHAR(50) NOT NULL DEFAULT 'Registered',     -- 'Registered', 'In Review', 'Escalated', 'Resolved'
    super_admin_notes TEXT,                               -- Private notes accessible exclusively to Super Admin
    email_dispatch_status VARCHAR(60) DEFAULT 'dispatched_yuvavaradhi1@gmail.com',
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Performance Indexes
CREATE INDEX IF NOT EXISTS idx_grievances_tracking_id ON public.grievances(tracking_id);
...
-- Enable PostgreSQL / Supabase Row-Level Security
ALTER TABLE public.grievances ENABLE ROW LEVEL SECURITY;

-- ----------------------------------------------------------------------------
-- RLS POLICIES FOR CITIZEN GRIEVANCE DESK
-- ----------------------------------------------------------------------------

-- Policy 1: Public & Authenticated Citizens can submit grievances (INSERT only)
CREATE POLICY "citizens_insert_grievances"
    ON public.grievances
    FOR INSERT
    TO public, anon, authenticated
    WITH CHECK (true);

-- Policy 2: Direct SELECT on unmasked grievances is locked EXCLUSIVELY to Super Admins
-- (Module Admins, standard users, and public visitors cannot query confidential records)
CREATE POLICY "super_admin_select_grievances"
    ON public.grievances
    FOR SELECT
    TO authenticated
    USING (
        EXISTS (
            ...
            WHERE profiles.id = auth.uid()
              AND profiles.role IN ('master_admin', 'super_admin')
        )
    );

-- Policy 3: Only Super Admins can UPDATE grievance status and resolution remarks
...
-- Policy 4: Zero-Delete Immutability Policy (Audited state records cannot be deleted)
CREATE POLICY "no_delete_grievances"
    ON public.grievances
    FOR DELETE
    USING (false);

-- Public Read-Only Function for Public Tracking (Returns strictly masked data)
CREATE OR REPLACE FUNCTION public.track_grievance_status(p_tracking_id VARCHAR)
RETURNS TABLE (
    tracking_id VARCHAR,
    category VARCHAR,
    district VARCHAR,
...
AS $$
    SELECT 
        ...
        g.district,
        g.status,
        COALESCE(g.citizen_name_masked, CONCAT(SUBSTRING(g.citizen_name FROM 1 FOR 2), '••••••••')) AS citizen_name_masked,
        COALESCE(g.citizen_phone_masked, CONCAT('+91 ••••• ••', RIGHT(g.citizen_phone, 2))) AS citizen_phone_masked,
        COALESCE(g.super_admin_notes, 'Under preliminary administrative verification.') AS super_admin_notes,
        g.created_at
    FROM public.grievances g
    WHERE g.tracking_id = UPPER(TRIM(p_tracking_id));
$$;
